...
from config import VG_IMAGES, IM_DATA_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN
...
class VG(Dataset):
    def __init__(self, mode, roidb_file=conf.custom_data, dict_file=VG_SGG_DICT_FN,
        ...
        self.split_mask, self.gt_boxes, self.gt_classes, self.relationships, self.imgs = load_graphs(
    ...
    def __getitem__(self, index):
        image_unpadded = Image.open(self.imgs[index]).convert('RGB')
        # image_unpadded = self.imgs[index]
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    boxes = []
    gt_classes = []
    for key in list(roi_h5['boxes'].keys()):
        imgs.append(str(np.array(roi_h5['imgs'][key])))
        boxes.append(np.array(roi_h5['boxes'][key]))
        gt_classes.append(np.array(roi_h5['labels'][key]))
